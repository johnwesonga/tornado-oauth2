...
class GoogleOAuth2Mixin(tornado.auth.OAuth2Mixin):
    ...
    def authorize_redirect(self, scope, **kwargs):
        args = {
          ...
          "client_id": settings.config['google_consumer_key'],
          "response_type": "code",
          "scope": scope
        }
    ...
    def get_authenticated_user(self, authorization_code, callback):
        args = {
            ...
            "client_secret": settings.config['google_consumer_secret'],
    ...
    def validate_token(self, session, callback):
        logging.info('validate token from url %s', self._OAUTH_TOKEN_VALIDATION_URL)
        self.httpclient_instance.fetch(
    ...
    def get_user_info(self, session, callback, response):
        GoogleOAuth2Mixin.access_token = session['access_token']
        logging.info('GoogleOAuth2Mixin.access_token: %s',GoogleOAuth2Mixin.access_token)
        additional_headers = {
        ...
        userjson = str(resp.read())
        print userjson
        user_info = escape.json_decode(userjson)
        callback(user_info)
